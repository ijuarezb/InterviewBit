...
class Solution:
    ...
    # Naive solution, using brute force, time complexity is O(n^3)
    def minimizeIJB(self, A, B, C):
    	la, lb, lc = len(A), len(B), len(C)
    	ia, ib, ic = 0, 0, 0
    	minmax = max(A) + max(B) + max(C)
    	current_max = 0

    	for ia in range(la):
    		for ib in range(lb):
    			for ic in range(lc):
    				current_max = max(abs(A[ia]-B[ib]), abs(B[ib]-C[ic]), abs(C[ic]-A[ia]))
