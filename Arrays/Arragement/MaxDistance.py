...
class Solution:
			...
			for j in range(i+1, len(A)):
				if A[i] <= A[j]:
					if max_distance < (j - i):
						...
						i_index = i
						j_index = j
					#elif max_distance == (A[j] - A[i]):
					#	j_index = j

		print("i_index: ", i_index, "A[i_index]: ",A[i_index])
		print("j_index: ", j_index, "A[j_index]: ",A[j_index])
	...
	def maximumGap2(self, A):
		...
		for i in range(len(A)):
			if temp > D[A[i]][0]:
				temp = D[A[i]][0]
			print("temp:{}; max_distance:{}".format(temp,max_distance))
			max_distance = max(max_distance, D[A[i]][-1] - temp)
...
if __name__ == "__main__":
	...
	print("the max distance is: ", sol.maximumGap2(A))
	A.sort()
	print(A[0],A[len(A)-1])
